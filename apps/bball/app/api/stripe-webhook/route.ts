		...
		if (event.type === "checkout.session.completed") {
			...
			if (metadata.status === "createTeam" && metadata.teamName !== "") {
				...
				const newTeam = new Team({
					...
					captainName: metadata.registrationName,
					registeredPlayers: [savedPlayer._id],
				});

				await newTeam.save();
				console.log("Registered team:", newTeam);
			...
			if (metadata.status === "joinTeam") {
				const registeredPlayer = new RegisterPlayer({
					season: metadata.season,
					status: metadata.status,
					division: metadata.division,
					registrationName: metadata.registrationName,
				...
				const savedPlayer = await registeredPlayer.save();
				console.log("Registered player:", registeredPlayer);

				const team = await RegisterTeam.findOneAndUpdate(
					{ teamName: metadata.teamName },
					{ $push: { registeredPlayers: savedPlayer._id } },
					{ new: true }
				);
				...
				if (metadata.payment === "four") {
					...
					schedule = await stripe.subscriptionSchedules.update(schedule.id, {
						...
						phases: [
							...
							{
								items: [
									{ price: "price_1Nt9kqHl6U3lbfQt2zcKK4Fv", quantity: 1 },
								],
								iterations: 1,
							},
						],
